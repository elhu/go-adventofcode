...
func decompress(data []byte) int {
	...
	for pos = 0; pos < len(data); {
		if data[pos] == '(' {
			...
			pos += mark.strLength
			subSize := decompress(data[pos : pos+mark.repeatLength])
			res += mark.repeatTimes * subSize
			pos += mark.repeatLength
		} else {
			res++
			pos++
		}
...
func main() {
	...
	check(err)
	fmt.Println(decompress([]byte(strings.Replace(strings.Trim(string(data), " \n"), " ", "", -1))))
}

...
func process(ipIndex int, insts []Instruction) []int {
	...
	for {
		...
		inst := insts[ip]
		if ip == 28 { // That's where we compare 5 and 0
			// Check if the values in register 5 ever repeat.
			// If they do, the previous one is the one we want
			if _, present := seen[registers[5]]; present {
				fmt.Printf("Max found: %d\n", prev)
				break
			}
			prev = registers[5]
			seen[registers[5]] = struct{}{}
		}

...
func paint(opcodes []int) map[string]int {
...
func solve(panels map[string]int) {
	whiteCoords := make([]coords, 0)
	for k, v := range panels {
		if v == 1 {
			whiteCoords = append(whiteCoords, keyToCoords(k))
		}
	}
	maxX, maxY := -1, -1

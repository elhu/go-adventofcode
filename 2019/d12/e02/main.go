...
func hash(moons []Moon) string {
	data := make([]string, 0, len(moons))
	for _, moon := range moons {
		data = append(data, fmt.Sprintf("%d:%d:%d.%d:%d:%d", moon.pos.x, moon.pos.y, moon.pos.z, moon.vec.x, moon.vec.y, moon.vec.z))
	}

...
func buildMap(opcodes []int) [][]byte {
	...
	for o := range io.output {
		if o == -99 {
			...
			if o == '\n' {
				...
				grid[index] = append(grid[index], byte(o))
			}
		}
	}
	return grid
...
func keyToCoords(s string) coords {
...
func findIntersections(grid [][]byte) map[string]struct{} {
	...
	for i := 1; i < len(grid)-1; i++ {
		for j := 1; j < len(grid[i])-1; j++ {
			if grid[i][j] == '#' &&
				...
				intersections[coordsToKey(coords{j, i})] = struct{}{}
				grid[i][j] = 'O'
			}
		}
	}

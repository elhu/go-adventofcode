...
func stateToKey(state *State) string {
	...
	return fmt.Sprintf("%s:%c:%c:%c:%c", string(collected), state.currNodes[0], state.currNodes[1], state.currNodes[2], state.currNodes[3])
}
...
func solve2(subMaps [4][][]byte, toCollect *byteset.ByteSet) int {
	var graphs [4]map[byte]*Node
	for i, m := range subMaps {
		graphs[i] = buildGraph(m)
	}
	pq := make(PriorityQueue, 1)
	var currNodes [4]*Node
	for i := 0; i < 4; i++ {
		currNodes[i] = graphs[i]['@']
	}
	pq[0] = &Item{
		value:    &State{collected: byteset.New(), distance: 0, currNodes: currNodes},
		priority: 0,
		index:    0,
	}
	heap.Init(&pq)
	...
	for pq.Len() > 0 {
		...
		for i, n := range state.currNodes {
			var newCurrNodes [4]*Node
			copy(newCurrNodes[:], state.currNodes[:])
			for key, dist := range reachableKeys(n, state.collected, 0, make(map[byte]int)) {
				newCollected := bsCopy(state.collected)
				newCollected.Add(key)
				newCurrNodes[i] = graphs[i][key]
				newState := &State{collected: newCollected, distance: state.distance + dist, currNodes: newCurrNodes}
				newStateKey := stateToKey(newState)
				if distance, exists := visited[newStateKey]; !exists || distance > newState.distance {
					visited[newStateKey] = newState.distance
					heap.Push(&pq, &Item{
...
func main() {
	...
	offsetX, offsetY := len(lines[0])/2, len(lines)/2
	boundaries := [4][2]coords2d.Coords2d{
		[2]coords2d.Coords2d{coords2d.Coords2d{X: 0, Y: 0}, coords2d.Coords2d{X: offsetX, Y: offsetY}},
		[2]coords2d.Coords2d{coords2d.Coords2d{X: offsetX, Y: 0}, coords2d.Coords2d{X: 2 * offsetX, Y: offsetY}},
		[2]coords2d.Coords2d{coords2d.Coords2d{X: 0, Y: offsetY}, coords2d.Coords2d{X: offsetX, Y: 2 * offsetY}},
		[2]coords2d.Coords2d{coords2d.Coords2d{X: offsetX, Y: offsetY}, coords2d.Coords2d{X: 2 * offsetX, Y: 2 * offsetY}},
	}

	var subMaps [4][][]byte
	for n, b := range boundaries {
		subMaps[n] = make([][]byte, offsetY+1)
		for i := b[0].Y; i <= b[1].Y; i++ {
			subMaps[n][i-b[0].Y] = make([]byte, offsetX+1)
			copy(subMaps[n][i-b[0].Y], lines[i][b[0].X:b[1].X+1])
		}
	}
	fmt.Println(solve2(subMaps, listKeys(lines)))
}

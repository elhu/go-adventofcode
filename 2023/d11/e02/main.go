...
func findExpandableCols(starMap []string) *set.Set[int] {
	ec := set.New[int]()
...
func findExpandableRows(starMap []string) *set.Set[int] {
	er := set.New[int]()
...
func distance(left, right coords2d.Coords2d, ec, er *set.Set[int]) int {
	added := 0
	...
	return added*(1000000-1) + coords2d.Distance(left, right)
}

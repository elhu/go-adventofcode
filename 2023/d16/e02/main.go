...
func process(grid []string, startBeam *Beam) int {
...
func solve(grid []string) int {
	...
	for i := 1; i < len(grid)-1; i++ {
		p := process(grid, &Beam{active: true, dirIdx: 1, pos: coords2d.Coords2d{X: 0, Y: i}})
		if p > max {
			max = p
		}
		p = process(grid, &Beam{active: true, dirIdx: 3, pos: coords2d.Coords2d{X: len(grid) - 1, Y: i}})
		if p > max {
			max = p
		}
	}
	for i := 1; i < len(grid[0])-1; i++ {
		p := process(grid, &Beam{active: true, dirIdx: 2, pos: coords2d.Coords2d{X: i, Y: 0}})
		if p > max {
			max = p
		}
		p = process(grid, &Beam{active: true, dirIdx: 0, pos: coords2d.Coords2d{X: i, Y: len(grid[0]) - 1}})
		if p > max {
			max = p
		}
	}

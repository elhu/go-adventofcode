...
func bubbleSort(list []int, ltgt map[int]*sets.Set[int]) []int {
	swaps := 1
	for swaps != 0 {
		swaps = 0 // reset swaps
		for i := 0; i < len(list)-1; i++ {
			if ltgt[list[i]] == nil || !ltgt[list[i]].HasMember(list[i+1]) {
...
func solve(ltgt map[int]*sets.Set[int], lists [][]int) int {
	...
	for _, list := range lists {
		if !isOrdered(ltgt, list) {
			ordered := bubbleSort(list, ltgt)
			res += ordered[len(list)/2]
		}
	}
	return res
}

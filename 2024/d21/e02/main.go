...
func solveDirPads(targetSeq string, dirPadIdx int, cache map[State]int) int {
	if val, found := cache[State{Seq: targetSeq, Depth: dirPadIdx}]; found {
		return val
	}
	...
	for _, part := range parts[:len(parts)-1] {
		c := solveDirPads(part+"A", dirPadIdx-1, cache)
		cache[State{Seq: part + "A", Depth: dirPadIdx - 1}] = c
		count += c
	}
	return count
}
...
func solve(codes []string) int {
	...
	for _, code := range codes {
		...
		count := solveDirPads(numSeq, 25, make(map[State]int))
